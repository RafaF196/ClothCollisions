...
public class Particle {
	...
	private Vector3f getSpringForceStreach(ArrayList<Particle> particles, Integer index, Float ke, Float kd, Float L, Integer numParticles) {
		Vector3f totalForce = new Vector3f(0,0,0);
		// Left side
		if (index%numParticles > 0) {
			P1.set(particles.get(index - 1).getPrevPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index - 1).getOldVelocity()); // Hay que pillar velocidad no cambiada
		...
		// Right side
		if (index%numParticles < numParticles - 1) {
			P1.set(particles.get(index + 1).getPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index + 1).getVelocity());
			v2.set(this.getVelocity());
			Vector3f.sub(P1, P2, auxVec3);
			Vector3f.sub(v1, v2, auxVec3b);
			Float aux2 = (kd*Vector3f.dot(auxVec3, auxVec3b))/auxVec3.length();
		...
		if (index > numParticles - 1) {
			P1.set(particles.get(index - numParticles).getPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index - numParticles).getVelocity());
			v2.set(this.getVelocity());
			Vector3f.sub(P1, P2, auxVec3);
			Vector3f.sub(v1, v2, auxVec3b);
			Float aux2 = (kd*Vector3f.dot(auxVec3, auxVec3b))/auxVec3.length();
		...
		// Bottom side
		if (index < particles.size() - numParticles) {
			P1.set(particles.get(index + numParticles).getPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index + numParticles).getVelocity());
			v2.set(this.getVelocity());
	...
	private Vector3f getSpringForceShear(ArrayList<Particle> particles, Integer index, Float ke, Float kd, Float L, Integer numParticles) {
		Vector3f totalForce = new Vector3f(0,0,0);
		L *= (float) Math.sqrt(2.0);
		// UpperLeft side
		if (index%numParticles > 0 && index > numParticles - 1) {
			P1.set(particles.get(index - numParticles - 1).getPrevPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index - numParticles - 1).getOldVelocity()); // Hay que pillar velocidad no cambiada
			v2.set(this.getVelocity());
			Vector3f.sub(P1, P2, auxVec3);
			Vector3f.sub(v1, v2, auxVec3b);
			Float aux2 = (kd*Vector3f.dot(auxVec3, auxVec3b))/auxVec3.length();
		...
		// UpperRight side
		if (index%numParticles < numParticles - 1 && index > numParticles - 1) {
			P1.set(particles.get(index - numParticles + 1).getPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index - numParticles + 1).getVelocity());
			v2.set(this.getVelocity());
			Vector3f.sub(P1, P2, auxVec3);
			Vector3f.sub(v1, v2, auxVec3b);
			Float aux2 = (kd*Vector3f.dot(auxVec3, auxVec3b))/auxVec3.length();
		...
		// BottomLeft side
		if (index%numParticles > 0 && index < particles.size() - numParticles) {
			P1.set(particles.get(index + numParticles - 1).getPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index + numParticles - 1).getVelocity());
			v2.set(this.getVelocity());
		...
		// BottomRight side
		if (index%numParticles < numParticles - 1 && index < particles.size() - numParticles) {
			P1.set(particles.get(index + numParticles + 1).getPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index + numParticles + 1).getVelocity());
			v2.set(this.getVelocity());
	...
	private Vector3f getSpringForceBend(ArrayList<Particle> particles, Integer index, Float ke, Float kd, Float L, Integer numParticles) {
		Vector3f totalForce = new Vector3f(0,0,0);
		L *= 2;
		// Left side
		if (index%numParticles > 1) {
			P1.set(particles.get(index - 2).getPrevPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index - 2).getOldVelocity()); // Hay que pillar velocidad no cambiada
			v2.set(this.getVelocity());
			Vector3f.sub(P1, P2, auxVec3);
			Vector3f.sub(v1, v2, auxVec3b);
			Float aux2 = (kd*Vector3f.dot(auxVec3, auxVec3b))/auxVec3.length();
		...
		// Right side
		if (index%numParticles < numParticles - 2) {
			P1.set(particles.get(index + 2).getPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index + 2).getVelocity());
			v2.set(this.getVelocity());
			Vector3f.sub(P1, P2, auxVec3);
			Vector3f.sub(v1, v2, auxVec3b);
			Float aux2 = (kd*Vector3f.dot(auxVec3, auxVec3b))/auxVec3.length();
		...
		// Top side
		if (index > 2*numParticles - 1) {
			P1.set(particles.get(index - 2*numParticles).getPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index - 2*numParticles).getVelocity());
			v2.set(this.getVelocity());
			Vector3f.sub(P1, P2, auxVec3);
			Vector3f.sub(v1, v2, auxVec3b);
			Float aux2 = (kd*Vector3f.dot(auxVec3, auxVec3b))/auxVec3.length();
		...
		// Bottom side
		if (index < particles.size() - 2*numParticles) {
			P1.set(particles.get(index + 2*numParticles).getPosition());
			P2.set(this.getPosition());
			v1.set(particles.get(index + 2*numParticles).getVelocity());
			v2.set(this.getVelocity());
	...
	public boolean update(ArrayList<Particle> particles, Float L, Integer index, Float delta, CollisionMaster cm,
		...
		Vector3f.add(force, getSpringForceStreach(particles, index, 1.0f, 0.01f, L, numParticles), force);
		Vector3f.add(force, getSpringForceBend(particles, index, 0.6f, 0.005f, L, numParticles), force);
		Vector3f.add(force, getSpringForceShear(particles, index, 0.2f, 0.003f, L, numParticles), force);
